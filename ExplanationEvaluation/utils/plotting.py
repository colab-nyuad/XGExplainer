...
def plot_ba2(graph, edge_weights, idx):
    # Calculate the thresholds for color assignment
    edge_weights = edge_weights.detach()
    num_edges = edge_weights.shape[0]
    threshold_low = np.percentile(edge_weights, 33)
    threshold_high = np.percentile(edge_weights, 66)
    ...
    # Add edges to the graph and assign colors based on thresholds
    for i in range(graph.shape[1]):
        edge = tuple(graph[:, i])
        weight = edge_weights[i]
        G.add_edge(int(edge[0]), int(edge[1]), weight=weight)

        # Assign color based on weight thresholds
        if weight >= threshold_high:
            G[int(edge[0])][int(edge[1])]['color'] = 'black'
        elif weight >= threshold_low:
            G[int(edge[0])][int(edge[1])]['color'] = 'grey'
        else:
            G[int(edge[0])][int(edge[1])]['color'] = 'gainsboro'

    # Draw the edges with assigned colors
    edge_colors = [G[u][v]['color'] for u, v in G.edges()]

    # Create the plot
    pos = nx.kamada_kawai_layout(G)
        
    nx.draw_networkx_edges(G, pos, edge_color=edge_colors, width=4)
    nx.draw_networkx_nodes(G, pos, node_color='royalblue', node_size=150,edgecolors='black')
    
    # Show the plot
    plt.axis('off')
    save_path = f'./qualitative/xgexplainer/ba2motifs/'
...
def plot(graph, edge_weigths, labels, idx, thres_min, thres_snip, dataset, args=None, gt=None, show=False):
    if args.dataset == "ba2":
        plot_ba2(graph, edge_weigths, idx)
        return
    """
