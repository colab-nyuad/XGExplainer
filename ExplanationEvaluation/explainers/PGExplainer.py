...
class PGExplainer(BaseExplainer):
    ...
    """
    def __init__(self, model_to_explain, graphs, features, task, device = torch.device('cpu'), epochs=30, lr=0.003, temp=(5.0, 2.0), reg_coefs=(0.05, 1.0),sample_bias=0, hops=3):
    ...
    def train(self, indices = None):
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            for n in indices:
                ...
                masked_pred = self.model_to_explain(feats, graph, edge_weights=mask)    ##FORWARD PASS, POTENTIAL TO USE GPU HERE
                original_pred = self.model_to_explain(feats, graph) ## ANOTHER FORWARD PASS
    ...
    def inductive_explain(self, index, node_idx = None):
        ...
        if self.type == 'graph':
            feats = self.ood_features[index].clone().detach() # use the ood features for graph
        else:
            feats = self.features # use original features for node classification (as there is no difference in message propagation to the node as long as there are no connecting edges)
        embeds = self.model_to_explain.embedding(feats, graph).detach()

        # Use explainer mlp to get an explanation
        input_expl = self._create_explainer_input(graph, embeds, node_idx).unsqueeze(dim=0)
        sampling_weights = self.explainer_model(input_expl)
        mask = self._sample_graph(sampling_weights, training=False).squeeze()
