...
def load_real_dataset(path_npy, path_graph):
    """Obtain the mutagenicity dataset from text files.
    
    :param path_npy: Path to save the numpy file containing the mutagenicity dataset.
    ...
    for node_label in tqdm(node_label_lists):
    ...
    print("Serializing the Numpy objs to File...")
    with open(path_npy, 'wb') as fout:
        np.save(fout, adjs)
        np.save(fout, feas)
        np.save(fout, labels)
