...
def load_mutag_ground_truth(shuffle=True):
    ...
    if not os.path.exists(path): # npy not yet created
        ...
        with open(path, 'rb') as fin:
            original_adjs = np.load(fin)
            original_features = np.load(fin)
            original_labels = np.load(fin)
...
def load_dataset_ground_truth(_dataset, test_indices=None):
    ...
    elif _dataset =="reddit":
        _, _, _, _, _, test_mask = load_dataset(_dataset)
        test_indices = np.argwhere(test_mask).squeeze()
        test_indices = test_indices.tolist()
        sampled_test_indices = test_indices[::5]
        return None, sampled_test_indices
