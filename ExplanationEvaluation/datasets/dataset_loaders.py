...
def load_graph_dataset(_dataset, shuffle=True):
    ...
    if _dataset == "ba2":
        ...
        '''
        Using pickle module to dump the adjs, features, and labels leads to an error, as pickle cannot serialize a bytes object larger than 4 GiB
        Using Numpy Save instead
        '''
        dir_path = os.path.dirname(os.path.realpath(__file__))
        # path = dir_path + '/pkls/' + "Mutagenicity" + '.pkl'
        path = dir_path + '/pkls/' + "Mutagenicity" + '.npy'
        if not os.path.exists(path): # npy not yet created
            print("Mutag dataset numpy file is not yet created, doing this now. Can take some time")
            ...
            try:
                ...
                raise Exception("File Corrupted (possibly due to unfinished dataset creation), Please delete ExplanationEvaluation/datasets/pkls/Mutagenicity.npy and Rerun")
    else:
...
def load_dataset(_dataset, skip_preproccessing=False, shuffle=True):
    ...
    :param larger_graphs: whether to only use larger graphs
...
def get_data_list(dataset, mask):
    '''
    gets the list of graphs, used for preparing reddit dataset
    also add constant edge weights
    '''
